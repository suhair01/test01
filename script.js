...
if (window.ethereum) {
  window.ethereum.on("accountsChanged", async () => {
    connect(); // Reconnect when user changes wallet
  });

  window.ethereum.on("chainChanged", () => {
    location.reload(); // Reload page on network change
  });
}
...
async function connect() {
  ...
  try {
    ...
    router = new ethers.Contract(routerAddress, ABI, signer); // For swap
    arenaRouter = new ethers.Contract(arenaRouterAddress, ABI, provider); // For estimation

    const btn = document.querySelector(".connect-btn");
    const short = `${userAddress.slice(0, 6)}...${userAddress.slice(-4)}`;
    btn.innerHTML = `${short} <span onclick="copyAddress(event)">📋</span>`;
    btn.disabled = true;
